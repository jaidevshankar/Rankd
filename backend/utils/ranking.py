...
def get_ranking(user_id: int, topic_id: Optional[int] = Query(None)):
    """
    Fetch the rankings for a specific user and topic.

    :param user_id: ID of the user.
    :param topic_id: ID of the topic (optional).
    :return: List of rankings for the given user and topic.
    """
    if topic_id is None:
        raise HTTPException(status_code=400, detail="topic_id is required")

    # Query rankings for the given user_id and topic_id
    rankings = supabase.table("Rankings").select("*").eq("user_id", user_id).eq("topic_id", topic_id).execute().data

    if not rankings:
        raise HTTPException(status_code=404, detail="No rankings found for this user and topic")

    # Fetch the ordered rankings using linked list logic
    ordered_rankings = fetch_ranked_items(user_id, topic_id)

    return {"user_id": user_id, "topic_id": topic_id, "rankings": ordered_rankings}